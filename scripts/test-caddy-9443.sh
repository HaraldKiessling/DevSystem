#!/bin/bash
#
# DevSystem - E2E Test für Caddy (Port 9443)
# Autor: DevSystem Team
# Datum: 2026-04-08
#
# Beschreibung: 
# Dieses Skript führt End-to-End-Tests für die Caddy-Installation und -Konfiguration
# mit nicht-Standard-Port 9443 durch.
# Es überprüft:
# - Ob Caddy installiert ist und läuft
# - Ob Caddy auf dem richtigen Port (9443) lauscht
...
while [[ "$#" -gt 0 ]]; do
    case $1 in
        ...
        --help) 
            ...
            echo "  --caddy-https-port=PORT   Port für Caddy HTTPS (Standard: 9443)"
            exit 0
            ;;
...
echo -e "${BLUE}=======================================================${NC}"
echo -e "${BLUE}     DevSystem E2E Test: Caddy (Port ${CADDY_HTTPS_PORT})${NC}"
echo -e "${BLUE}=======================================================${NC}"
...
for dir in "$CADDY_DIR/tls" "$CADDY_DIR/tls/tailscale" "$CADDY_DIR/tls/local" "$CADDY_LOG_DIR"; do
    total_tests=$((total_tests+1))
...
# 3.2: Monitoring-Skript prüfen
total_tests=$((total_tests+1))
if ! check_file_exists "/usr/local/bin/caddy-monitor.sh"; then
    failed_tests=$((failed_tests+1))
fi

# 3.3: Zertifikatserneuerungsskript prüfen
...
# 5.2: Prüfen, ob Caddy auf dem richtigen Port lauscht
total_tests=$((total_tests+1))
if ! check "Caddy lauscht auf HTTPS-Port (${CADDY_HTTPS_PORT})" "ss -tulpn | grep LISTEN | grep caddy" "${CADDY_HTTPS_PORT}"; then
    failed_tests=$((failed_tests+1))
fi
...
if ip a | grep -q "tailscale"; then
    ...
    if [[ -n "$TAILSCALE_IP" ]]; then
        if ! check "Zugriff über Tailscale funktioniert" "curl -s --resolve \"$TS_DOMAIN:${CADDY_HTTPS_PORT}:$TAILSCALE_IP\" \"https://$TS_DOMAIN:${CADDY_HTTPS_PORT}/\" -k" "Test Caddy Reverse Proxy"; then
...
total_tests=$((total_tests+1))
if ! check "Zugriffseinschränkung funktioniert" "curl -s -H 'X-Forwarded-For: 1.2.3.4' \"http://localhost:${CADDY_HTTPS_PORT}/\" -k" "Zugriff nur über Tailscale erlaubt"; then
    failed_tests=$((failed_tests+1))
fi

# 7.4: Prüfen, ob der Caddy-Port in der Firewall geöffnet ist
total_tests=$((total_tests+1))
if command -v ufw &> /dev/null && ufw status | grep -q "active"; then
    if ! check "Port ${CADDY_HTTPS_PORT} ist in der Firewall geöffnet" "ufw status | grep ${CADDY_HTTPS_PORT}" "ALLOW"; then
        failed_tests=$((failed_tests+1))
    fi
else
    echo -e "${YELLOW}[INFO]${NC} UFW ist nicht aktiv oder nicht installiert. Firewall-Test übersprungen."
fi
